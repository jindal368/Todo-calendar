...
const Week = () => {
  ...
  const renderDayHeaders = () => {
    ...
    return <div className="flex flex-row justify-evenly ml-48">{headers}</div>;
  };

  const handlePrevThreeDays = () => {
    setCurrentStartDay((prevDay) => addDays(prevDay, -7));
  };
  ...
  return (
    <CalendarWrapper>
      <Navigation>
        <Button onClick={handlePrevThreeDays}>Previous 7 Days</Button>
        <Button onClick={handleNextThreeDays}>Next 7 Days</Button>
      </Navigation>
      <WeekGrid>
        {renderDayHeaders()}
        {renderHours()}
